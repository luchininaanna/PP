...
vector<vector<int>> Matrix::GetAdditionMatrix() {
	...
	int threadAmount = 2;
	vector<vector<vector<int>>> portions = Devide(dimension, threadAmount);

	for (int i = 0; i < threadAmount; i++) {

		DataForThread currData;
		currData.matrixPtr = this;
		currData.newMatrix;
		InitializeAddMatrix(currData.newMatrix);

		currData.portion = portions[i];
		cout << currData.portion.size() << endl;

		hThread[i] = CreateThread(NULL, 2048, &GetMinor, (PVOID)&currData, 0, NULL);
	}

	WaitForMultipleObjects(threadAmount, hThread, true, INFINITE);
...
DWORD WINAPI Matrix::GetMinor(LPVOID lpParam) {
	DataForThread *currData = (DataForThread*)lpParam;
	Matrix* matrixPtr = currData->matrixPtr;

	vector<vector<int>> portion = currData->portion;
